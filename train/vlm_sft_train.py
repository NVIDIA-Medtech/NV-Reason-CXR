...
def main(script_args, training_args, model_args):
    ...
    features = datasets.Features(
        {
            ...
            "messages": datasets.List(
                {"role": Value("string"), "content": datasets.List({"type": Value("string"), "text": Value("string"), "image": Value("string")})}
            ),
        }
    )
    ...
    my_dataset = datasets.load_dataset(
        "json",
        data_files=os.path.join(script_args.dataset_root, script_args.dataset_path),
        split="train",
    ...
    processor_config = {}
    if (
        isinstance(model, Qwen2_5_VLForConditionalGeneration)
        and script_args.min_image_height is not None
        and script_args.max_image_height is not None
    ):
